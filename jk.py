...
print(lista_empleados) # con esta linea imprimimos lo que hay dentro de la capeta "Empleados" en forma de lista
## ['Empleado_z.jpg', 'Empleado_y.jpg', 'Empleado_x.jpg'........]
# decimos que por cada nombre(archivo de imagen) que encuentre en la carpeta de empleados
for nombre  in lista_empleados:
    if nombre.startswith('.'): ## si alguno de esos archivos empeiza con "."
        continue# no lo tome en cuenta y continue con las siguientes instrucciones
    ...
    if imagen_actual is None: ## si la imagen, no pudo ser leida
        print(f'Error al leer {nombre}')##entonces mostramos en pantalla, el nombre(elemento) que no pudo ser leida
        continue
    else: ## entonces cuando es posible leer una imagen, se mostrara el siguiente mensaje
...
time.sleep(2) # aqui le pedimos que espere 2 segundos antes de hacer la captura,si no ponemos esta linea la imagen que se captura es negra(recien se abre la camara)
## Esto permite que la cámara ajuste automáticamente
...
print(exito) # aquí vamos a imprimir un boooleano,es decir nos indica con un "True" si la captura fue tomada o con un "False" si la captura no fue tomada

cv2.imshow('Prueba', imagen) # entoces con ayuda de la libreria "cv2" y su metodo ".imshow" vamos a mostrar una 1era imagen de prueba
cv2.waitKey(0)# y le vamos a pedi que muestre esa imagen de prieba, hasta que el usuario oprima otra letra

if not exito:
    print("No se a podido tomar la captura") ### en dado caso no haber podido tomar la captura, se mostrara este mensaje en consola
else:

    ## reconocer cara/ rostro en la captura, es decir obetenemos la ubicación del rostro/ cara
    cara_captura = fr.face_locations(imagen)
    ## codificar cara capturada
    print(cara_captura)# imprimimos las cordenadas
    ...
    print(len(cara_captura_codificada))# imprimimos cuantas caras fueron decodificadas dentro de esta imagen o captura
